...
var squares_left = 9 #Untapped squares
...
func _ready():
	...
	llevel = get_node("/root/data").llevel
...
func _getReferences():
	...
	board = get_node("board_pos/board")
...
func _save_level():
	get_node("/root/data").llevel.score=score
	get_node("/root/data").llevel.lvl=minimap.level
	get_node("/root/data").llevel.exists=1
	get_node("/root/data").llevel.colors=board._getColors()
	get_node("/root/data").llevel.colcount=color_count
	get_node("/root/data").llevel.shape=shape
	
	get_node("/root/data").save_level(false)
	llevel = get_node("/root/data").llevel
...
func _checkColor(square):
	...
	if squares_left == 0 && started:
		...
		if score_now==9:
			...
			get_node("sqoin/aplayer").play("pop")
		...
		if ldata.record<score:
			get_node("/root/data").load_data.record=score
...
func _on_yes_pressed():
	...
	var nshape = load("res://scenes/shapes/shape"+str(llevel.shape)+".tscn").instance()
...
func _on_skip_pressed():
	...
	if ldata.ads == 1:
		var action=false
		if score<minimap.leveling[0] && ldata.record>=minimap.leveling[0]:
			action=true
			score=minimap.leveling[0]
			_next_match()
		elif score>=minimap.leveling[0] && score<minimap.leveling[1] && ldata.record>=minimap.leveling[1]:
			action=true
			score=minimap.leveling[1]
			_next_match()
		elif score>=minimap.leveling[1] && score<minimap.leveling[2] && ldata.record>=minimap.leveling[2]:
			action=true
			score=minimap.leveling[2]
			_next_match()
		elif score>=minimap.leveling[2] && score<minimap.leveling[3] && ldata.record>=minimap.leveling[3]:
			action=true
			score=minimap.leveling[3]
			_next_match()
		elif score>=minimap.leveling[3] && score<minimap.leveling[4] && ldata.record>=minimap.leveling[4]:
			action=true
